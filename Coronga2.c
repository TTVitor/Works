...
void trocaCaracteres(char* var, int tipo) {
	int i;
...
int recuperaDadosTexto() {
	...
	// Sucesso
		...
		for (i = 0; i < qtde; i++) {
			...
			trocaCaracteres(&dadosPacientes[i].cidade, 2);
			trocaCaracteres(&dadosPacientes[i].nome, 2);
		}
	}
...
int gravaDadosnoTxt(int qtde) {
	...
	// Sucesso
		...
		fprintf(arqPacientes, "\n");
	   	...
	   	for (i = 0; i < qtde; i++) {
	   		trocaCaracteres(&dadosPacientes[i].cidade, 1);
	   		trocaCaracteres(&dadosPacientes[i].nome, 1);
...
int gravaDadosnoBin(int qtde) {
	int i, erro;
	
	// Erro
	if ((arqPacientes = fopen("pacientes.dat", "ab")) == NULL) {
		return 0; 
		exit(1);
	}
	...
	// Sucesso
		...
		fwrite(&dadosPacientes,sizeof(dadosPacientes),1,arqPacientes);	
	}	
...
void menu(int qtde, int tam) {
	...
	do {
		...
		printf(" |  GRAVA EM TXT     |     7      |\n");
		printf(" |  GRAVA EM BIN     |     8      |\n");	
		printf(" |  PESQUISA TXT     |     9      |\n");					
		printf(" |--------------------------------|\n");		
		printf(" |  SAIR DO PROGRAMA |     10     |\n");
	...
	switch(op) {
		...
		case 5:
			...
			break;
		case 6:
			if (qtde = recuperaDadosTexto()) printf("Dados recuperados do arquivo com sucesso!\n");
			...
			break;
		case 7:
			if (gravaDadosnoTxt(qtde)) printf("Dados gravados no arquivo com sucesso!\n");
			...
			break;
		case 8:
			if (gravaDadosnoBin(qtde)) printf("Dados gravados no arquivo com sucesso!\n");
			else printf("Erro ao tentar abrir arquivo!\n");
...
void pesquisaArquivoTxt(int qtde) {
	int buscar, flag, total, i;
	char voltar;
	struct FichaPacientes paciente;
	...
	if ((arqPacientes = fopen("pacientes.txt", "r")) == NULL) {
		...
		fscanf(arqPacientes, "%d", &total);
		
		buscar = dadosPacientes[qtde].codigo;
		
		while (!feof(arqPacientes)) {
			fscanf(arqPacientes, "%d %d %i %i %i %i %i %i %i %i %i %i %s %s %c", &paciente.codigo, &paciente.idade, &paciente.sintomas[0], &paciente.sintomas[1], &paciente.sintomas[2], &paciente.sintomas[3], &paciente.sintomas[4], &paciente.sintomas[5], &paciente.sintomas[6], &paciente.sintomas[7], &paciente.sintomas[8], &paciente.sintomas[9], &paciente.nome, &paciente.cidade, &paciente.sexo);
			
			if (paciente.codigo == buscar) {	
				flag = 1;
				printf("O paciente foi encontrado dentro do arquivo txt...\n\n");
				
				printf("\nCódigo: %i", paciente.codigo);
				...
				printf("\nSintomas:\n");
				for (i = 0; i < 10; i++) {
					if (paciente.sintomas[i] == 1) printf("- FEBRE\n");
					if (paciente.sintomas[i] == 2) printf("- DORES NO CORPO\n");
					if (paciente.sintomas[i] == 3) printf("- CORIZA\n");
					if (paciente.sintomas[i] == 4) printf("- TOSSE\n");
					if (paciente.sintomas[i] == 5) printf("- DIARRÉIA\n");
					if (paciente.sintomas[i] == 6) printf("- DOR DE CABEÇA\n");
					if (paciente.sintomas[i] == 7) printf("- FALTA DE AR\n");
					if (paciente.sintomas[i] == 8) printf("- DOR DE GARGANTA\n");
				}
		...
		if (!flag) printf("Não foi encontrado o paciente no arquivo txt...\n\n");
	}
