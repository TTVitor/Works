...
void consultaProduto(int qtd, struct dMedicamentos dadosMed[]) {
	int i, j, pesq, op, chave;
	float rcVlr;
	int flag, ini, meio, fim, contador;
	
	// Ordenação do vetor de código de medicamento c/ Insertion Sort
 	for ( i = 1; i < qtd; i++ ) {
		chave = dadosMed[i].codMedicamento;
		j = i - 1;
		while (j >= 0 && dadosMed[j].codMedicamento > chave ) {
			dadosMed [j +1].codMedicamento = dadosMed[j].codMedicamento;
			j = j - 1;
			dadosMed[j+1].codMedicamento = chave;
		}
	}
	//
	...
	do{
		if(pesq>dadosMed[meio].codMedicamento){
			...
			if(pesq<dadosMed[i].codMedicamento){
				fim=meio-1;
				meio=(ini+fim)/2;
			}else
				flag=1;
	...
	for(i = 0; i < qtd; i++) {
		if(pesq == dadosMed[i].codMedicamento) {
			textcolor(LIGHTMAGENTA);
			printf("Nome do Medicamento: %s\n", dadosMed[i].descMedicamento);
			printf("Código do Medicamento: %i\n", dadosMed[i].codMedicamento);
			printf("\n");
			textcolor(WHITE);
			if (dadosMed[i].tpMd==1) {
				printf("Tipo de Medicamento: Comprimido/Capsulas \n\n");
				textcolor(WHITE);
				printf("Quantia de COMPRIMIDOS presentes na Cartela: %i \n", dadosMed[i].qtComp);
				printf("Quantia de MILIGRAMAS presentes nos Comprimidos: %i \n\n", dadosMed[i].qtMg);
			}else
				if (dadosMed[i].tpMd==2) {
					printf("Tipo de Medicamento: Xarope/Soluções \n\n");
					textcolor(WHITE);
					printf("Quantia de MILILITROS: %i Ml \n", dadosMed[i].qtMl);
				}
			textcolor(LIGHTMAGENTA);
			printf("\n");
			printf("Quantia do Medicamento disponível: %i \n\n", dadosMed[i].disponibilidade);
			textcolor(WHITE);
			printf("Valor do Medicamento (UND): R$ %.2f\n", dadosMed[i].vlrMedicamento);	
		}
	}
}
